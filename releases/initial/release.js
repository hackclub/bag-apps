...
export async function setup() {
  ...
  return new Promise((resolve, reject) => {
    ...
    const server = app.listen(5000, () => open(authorizeUrl, { wait: false }))
            ...
            if (row.length >= 8) {
              ...
              try {
                ...
                const item = await app.updateItem({
                  itemId: name,
                  new: {
                    metadata: JSON.stringify({ rarity: Number(rarity) })
                  }
                })
...
async function main() {
  ...
  parse().then(async slackers => {
    ...
    for (let slacker of slackers) {
      if (blacklist.includes(slacker)) continue
      const { id } = slacker
      let given = []
      let note = `An mysterious old man comes by your home with a jaunty stroll, whistling as he goes. He throws a bag at your door, which bounces off and lands on your doorstep. You pick the bag up, feeling three objects inside. Some instructions are printed on the outside: *This is your bag. Use \`/bag\` to see what it holds.*`
      for (let item of items.sort((a, b) => (Math.random() < 0.5 ? 1 : -1))) {
        const prob = Math.random()
        if (prob < item.metadata.rarity) {
          given.push({ itemId: item.name, quantity: 1 })
    ...
    const task = cron.schedule('0 10 * * *', async () => {
      ...
      for (let slacker of slackers) {
        const { id } = slacker
        for (let item of items.sort(() => (Math.random() < 0.5 ? 1 : -1))) {
          const prob = Math.random()
          if (prob < item.metadata.rarity) {
            let note
            switch (day) {
              case 2:
                note = `The old man appears again, kicking up dust as he shuffles down the road. He carefully places ${item.reaction} on your doorstep. You are suddenly overcome with the urge to type \`/item ${item.name}\` to learn about this curious object.`
                break
              case 3:
